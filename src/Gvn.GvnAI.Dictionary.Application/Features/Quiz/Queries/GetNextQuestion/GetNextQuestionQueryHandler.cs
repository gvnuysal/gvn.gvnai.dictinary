    ...
    IQuizService quizService) : IQueryHandler<GetNextQuestionQuery, QuizQuestionDto>
{
    public async Task<Result<QuizQuestionDto>> Handle(GetNextQuestionQuery request, CancellationToken cancellationToken)
    {
        ...
        var wordData = await quizService.GetRandomWordWithTranslationAsync(askedWordIds, cancellationToken);
        if (wordData is null)
        ...
        var wrongOptions = await quizService.GetRandomWrongOptionsAsync(wordId, 4, cancellationToken);
